package com.sudoajay.github_api.helper
...
open class InsetDivider : ItemDecoration() {
    ...
    private fun drawVertical(c: Canvas, parent: RecyclerView) {
        ...
        for (i in 0 until childCount) {
            ...
            val top: Int
            if (overlay) {
                bottom = child.bottom + params.bottomMargin + View.TRANSLATION_Y.get(child).roundToInt()
                top = bottom - dividerHeight
            } else {
                top = child.bottom + params.bottomMargin + View.TRANSLATION_Y.get(child).roundToInt()
    ...
    private fun drawHorizontal(c: Canvas, parent: RecyclerView) {
        ...
        for (i in 0 until childCount) {
            ...
            val left: Int
            if (overlay) {
                right = child.right + params.rightMargin + View.TRANSLATION_X.get(child).roundToInt()

                left = right - dividerHeight
            } else {
                left = child.right + params.rightMargin + View.TRANSLATION_X.get(child).roundToInt()
